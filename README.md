...
```properties
<quarkus.config-tracking.file-suffix>-ci</quarkus.config-tracking.file-suffix>
<quarkus.recorded-build-config.file>.quarkus/quarkus-prod-ci</quarkus.recorded-build-config.file>
```

It is also possible to precise an intermediate directory inside the `.quarkus` folder, in which the dump-config files are expected:
```properties
DEVELOCITY_QUARKUS_DUMP_CONFIG_INTERMEDIATE_DIRECTORY=ci
```
Doing so, this extension will look for the dump-config files in `.quarkus/ci` instead of `.quarkus`.
Worth noting that if not using the default value, the Quarkus property `quarkus.config-tracking.directory` should be set accordingly:
```properties
<quarkus.config-tracking.directory>.quarkus/ci</quarkus.config-tracking.directory>
```
This is required because it is the location in which the dump-config files are generated by Quarkus. Not doing so may prevent the extension from finding them.
