...
A version derived from a commit id puts the version in the cache key, so every commit re-runs `native-image` even when nothing the application builds from has changed. Measured on two builds differing only in version, it reaches six places in the jar the native image generation is keyed on:
